...
const EditNews = () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (newsData.image_url) {
          setImagePreview(`${API_BASE_URL}/${newsData.image_url}`);
        }
        
        if (newsData.author_image) {
          setAuthorImagePreview(`${API_BASE_URL}/${newsData.author_image}`);
        }
      } else {
        throw new Error(result.message || "Gagal mendapatkan data berita");
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('_method', 'PUT'); // Untuk Laravel yang menggunakan form method spoofing
      ...
      if (!response.ok) {
        if (result.errors) {
          ...
          throw new Error(result.message || "Gagal mengupdate berita");
        }
      }
      ...
      toast.error(err.message || "Terjadi kesalahan saat mengupdate berita");
    } finally {
      setLoading(false);
    }
  };
